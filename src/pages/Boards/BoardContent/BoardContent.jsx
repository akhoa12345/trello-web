...
function BoardContent({ board, createNewColumn, createNewCard, moveColumns, moveCardInTheSameColumn, moveCardToDifferentColumn }) {
  ...
  // Điểm va chạm cuối cùng trước đó (xử lí thuật toán phát hiện va chạm, video 37)
  ...
  const moveCardBetweenDifferentColumns = (
    ...
    setOrderedColumnsState(prevColumns => {
      ...
      if (triggerFrom === 'handleDragEnd') {
         ...
         */
        /**
         * Phải dùng tới activeDragItemData.columnId hoặc tốt nhất là oldColumnWhenDraggingCard._id (set vào
         * state từ bước handleDragStart) chứ không phải activeData trong scope handleDragEnd này vì sau khi
         * đi qua onDragOver và tới đây là state của card đã bị cập nhật 1 lần rồi.
         */
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.CARD) {
      ...
      if (oldColumnWhenDraggingCard._id !== overColumn._id) {
        ...
        // Vẫn gọi update state ở đây để tránh delay hoặc Flickering giao diện lúc kéo thả cần phải
  ...
  // column (video 37 fix bug quan trọng)
  ...
  const collisionDetectionStrategy = useCallback((args) => {
    ...
    if (overId) {
      ...
      // Nếu cái over nó là column thì sẽ tìm tới cái cardId gần nhất bên trong khu vực va chạm đó
      // dựa vào thuật toán phát hiện va chạm closestCorners hoặc closestCenter đều được. Tuy nhiên
